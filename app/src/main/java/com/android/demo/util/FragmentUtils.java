...
public final class FragmentUtils {
    ...
    private static Fragment operateFragment(@NonNull FragmentManager fragmentManager,
        ...
        if (srcFragment != null && srcFragment.isRemoving()) {
            Log.e(TAG, srcFragment.getClass().getSimpleName() + " is isRemoving");
            return null;
        }
    ...
    /**
     * 获取目标fragment的前(栈中上)一个fragment
     *
     * @param destFragment 目标fragment
     * @return 目标fragment的前（栈中上）一个fragment
     */
     ...
     * @return 是否消费回退事件
     */
    private static boolean dispatchBackPress(@NonNull FragmentManager fragmentManager) {
        List<Fragment> fragments = fragmentManager.getFragments();
        if (fragments == null || fragments.isEmpty())
            return false;
        for (int i = fragments.size() - 1; i >= 0; --i) {
...
//                view.setBackgroundResource(R.drawable.xxx);
                //Method below is deprecated. Replace it with the method above.
